...
import org.dzedzich.volunteers.profile.ui.views.impl.CompanyProfileActivity
...
class CompaniesAdapter : RecyclerView.Adapter<CompaniesAdapter.ViewHolder>() {
    ...
    private var list: MutableList<CompanyShort?> = ArrayList()
    ...
    fun loadList(list: List<CompanyShort?>) {
        this.list.clear()
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.rating.text = company?.rating.toString()
        holder.tasks.text = "${context?.resources?.getString(R.string.word_tasks)}: ${company?.taskCount}"
        holder.slogan.text = StringBuilder()
                                .append("${context?.resources?.getString(R.string.profile_company_atmosphere)}: ${company?.atmosphere} \n")
                                .append("${context?.resources?.getString(R.string.profile_company_conformity)}: ${company?.conformity} \n")
                                .append("${context?.resources?.getString(R.string.profile_company_organization_level)}: ${company?.organizationLevel}")
                                .toString()

        holder.card.onClick {
            val bundle = Bundle()
            bundle.putInt(Constants.BUNDLES.COMPANY_ID, company?.id as Int)
            bundle.putInt(Constants.BUNDLES.COMPANY_POSITION, position+1)
            context?.openScreen(CompanyProfileActivity::class.java, bundle)
        }
    }
